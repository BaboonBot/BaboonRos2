...
class yahboomcar_driver(Node):
	def __init__(self, name):
		...
		self.armPublisher = self.create_publisher(Float32MultiArray,"arm_joint_states",100)
	...
	def pub_data(self):
		...
		angles_int = self.car.get_uart_servo_angle_array()
		
		angle_list = Float32MultiArray()
		angle_list.data = [angle*1.0 for angle in angles_int]
		...
		self.armPublisher.publish(angle_list)
	...
	def on_ros_shutdown(self):
		self.car.set_auto_report_state(False, forever=False)
